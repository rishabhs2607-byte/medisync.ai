  ...
  if (user.role === "doctor" && user.status !== "approved") {
    return (
      <div className="min-h-screen bg-luxury-pureBlack flex items-center justify-center p-6 text-white relative">
        ...
        <div className="glass-panel p-8 rounded-3xl border border-luxury-goldRoyal/20 bg-luxury-richBlack/80 max-w-md text-center space-y-6 relative z-10 animate-glow-gold">
          <div className="w-16 h-16 bg-luxury-goldRoyal/10 border border-luxury-goldRoyal/30 rounded-2xl flex items-center justify-center text-luxury-goldRoyal mx-auto animate-pulse">
            <ShieldAlert size={28} />
          </div>

          <div className="space-y-2">
            <h2 className="text-xl font-extrabold tracking-wide uppercase text-white">CLINICAL LICENSE UNDER REVIEW</h2>
            <p className="text-xs text-zinc-400 leading-relaxed">
              Hello {user.name}. Your doctor registration application is currently under administrator review. 
              Clinicians must be verified before accessing patient telemetry databases.
            </p>
          </div>
          ...
          <div className="flex gap-4">
            <Link 
              href="/"
              className="w-full py-2.5 bg-luxury-goldRoyal text-luxury-pureBlack font-bold rounded-lg text-xs uppercase tracking-wider text-center"
            >
